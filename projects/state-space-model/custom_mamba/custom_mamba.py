...
class MixerModel(nn.Module):
    def __init__(
        ...
        if use_abs_position:
            ...
            self.freqs = torch.exp(-np.log(10000.0) / d_model * torch.arange(0, d_model, 2).float()).to(device=device)
    ...
    def forward(self, input_ids, position_ids=None, inference_params=None):
        ...
        if self.use_relative_position or self.use_abs_position:
            ...
            if self.use_abs_position:
                ...
                angles = position_ids.unsqueeze(-1) / self.freqs.to(position_ids.dtype).to(position_ids.device).unsqueeze(0)
