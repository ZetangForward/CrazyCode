...
torchrun --nnode=1 --nproc_per_node=$nproc_per_node --master_port 6989  mamba/train.py \
    mark=${mark} \
    model=$model_name \
    ...
    experiment.num_training_steps=10000 \
    experiment.warmup_steps=1000 \
    task.dataset.cluster_batch=False \
    task.dataset.train_batch_size=8 \
